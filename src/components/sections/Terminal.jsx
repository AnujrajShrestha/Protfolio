...
export default function Terminal({ repos, resync }) {
  const [lines, setLines] = useState([
    { text: "Welcome to Anuj's shell. Type <span class='text-wire'>help</span> to get started.", type: "out" },
  ]);
    ...
    <section className="border-t border-line/70 py-24">
      ...
      >
          ...
          {lines.map((l, i) => (
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: l.type === "cmd" ? `<span class="text-phosphor">visitor@Anuj:~$</span> ${l.text}` : l.text,
              }}
            />
          ))}
        </div>
        ...
        <div className="flex items-center gap-2 border-t border-line/80 bg-panel2 px-4 py-2.5">
          <span className="whitespace-nowrap text-[13px] text-phosphor">visitor@Anuj:~$</span>
          <input
            ref={inputRef}
            value={value}
